...
emitted once per session with the final count already computed.
...
## Status
Tested end-to-end in production-like conditions (not just manual
search): triggered for real on 2026-08-25 00:00:02, confirmed via
both the Triggered Alerts UI and the internal scheduler log
(`index=_internal sourcetype=scheduler`), with `result_count=1` and
throttling (`suppressed=1`) correctly engaging on the following run.
